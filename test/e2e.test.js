...
describe("E2E Test Scenarios", () => {
  describe("Issuing a single documents", () => {
    ...
    it("creates a signed document", () => {
      ...
      expect(signedDocument.schema).to.be.equal(
        "http://example.com/schema-v1.json"
      );
      ...
      expect(signedDocument.signature.merkleRoot).to.equal(
        signedDocument.signature.targetHash
      );
    });
  ...
  describe("Issuing a batch of documents", () => {
    ...
    it("does not allow for same merkle root to be generated", () => {
      ...
      expect(signedDocuments[0].signature.merkleRoot).to.not.equal(
        newSignedDocuments[0].signature.merkleRoot
      );
    });
  });
});

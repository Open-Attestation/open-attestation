...
describe("V4.0 sign", () => {
  it("should sign a VC", async () => {
    const signed = await signVc(RAW_VC_DID, SUPPORTED_SIGNING_ALGORITHM.Secp256k1VerificationKey2018, {
  ...
  it("should sign a VC with a wallet", async () => {
  ...
  it("should throw error if a signed VC is resigned", async () => {
    const signedVc = await signVc(RAW_VC_DID, SUPPORTED_SIGNING_ALGORITHM.Secp256k1VerificationKey2018, {

...
import { genTargetHash } from "../4.0/hash";
import { encodeSalt, salt } from "./salt";
import { ZodError } from "zod";

export const digestVC = async <T extends V4OpenAttestationDocument>(
  ...
  vc: NoExtraProperties<V4OpenAttestationDocument, T>
): Promise<V4WrappedDocument<T>> => {
  /* 1a. try OpenAttestation VC validation, since most user will be issuing oa v4*/
  const oav4context = await V4OpenAttestationDocument.pick({ "@context": true }).passthrough().safeParseAsync(vc); // Superficial check on user intention
  let validatedRawDocument: W3cVerifiableCredential | undefined;
  if (oav4context.success) {
    const oav4 = await V4OpenAttestationDocument.safeParseAsync(vc);
    if (!oav4.success) {
  ...
  const digest = genTargetHash(documentReadyForWrapping, salts, []);
...
export const digestVCs = async <T extends V4OpenAttestationDocument>(
  // NoExtraProperties prevents the user from passing in a document with extra properties, which is more aligned to our validation strategy of strict
  vcs: NoExtraProperties<V4OpenAttestationDocument, T>[]
): Promise<V4WrappedDocument<T>[]> => {

...
export async function validateW3C<T extends OpenAttestationDocument>(credential: WrappedDocument<T>): Promise<void> {
  ...
  // ensure validFrom is a valid RFC3339 date, see https://www.w3.org/TR/vc-data-model/#issuance-date
  if (credential.validFrom && !isValidRFC3339(credential.validFrom)) {
    throw new Error("Property 'validFrom' must be a valid RFC 3339 date");
  }
  // ensure validUntil is a valid RFC3339 date, see https://www.w3.org/TR/vc-data-model/#expiration
  if (credential.validUntil && !isValidRFC3339(credential.validUntil)) {
    throw new Error("Property 'validUntil' must be a valid RFC 3339 date");
  }
